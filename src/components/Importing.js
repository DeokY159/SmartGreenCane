import { SERVER_INFO, cane_info} from './config.js';


// fetchData 함수 정의 및 내보내기 -> becane임
...
// fetchData 함수 정의 및 내보내기 -> cane1임
export async function fetch_aeData(resource) {
    const url = `http://${cane_info.host}:${cane_info.port}${cane_info.basePath}/${resource}/la`;
    try {
        const response = await fetch(url, {
            method: 'GET',
            headers: {
